...
func (rf *Raft) genAppendEntiesParams() *AppendEntriesRequest {
...
func (rf *Raft) broadCastHeartBeat() {
	for peer := range rf.peers {
		...
		go rf.sendHeartBeatToFollower(peer)
	}
}

func (rf *Raft) sendHeartBeatToFollower(peer int) {
	if rf.state != Leader {
		return
	}

	req := rf.genAppendEntiesParams()

	response := new(AppendEntriesResponse)
	if rf.sendAppendEntries(peer, req, response) {
		rf.mu.Lock()
		rf.handleAppendEntriesResponse(peer, req, response)
		rf.mu.Unlock()
	}
}
